...
**注意**: 本文档主要介绍 Python 版本的故障排查。C++ 版本请参考 [C++ 实现指南](./CPP_IMPLEMENTATION.md)。
...
两个版本的 API 基本相同，常见问题也相似。主要差异：

| 问题类型 | Python | C++ | 解决方案 |
|---------|--------|-----|---------|
| 编译错误 | ❌ | ✅ | 见 C++ 指南 |
| 类型错误 | ⚠️ 运行时 | ✅ 编译时 | C++ 更早发现 |
| 内存问题 | 自动管理 | ⚠️ 需注意 | 使用智能指针 |
| 性能问题 | 更常见 | 较少见 | 考虑 C++ 版本 |
...
```cpp
try {
    JSONStore store("data.json", true);  // auto_create=true
    auto data = store.read();
}
catch (const FileNotFoundError& e) {
    auto data = store.read();  // 返回空对象
}
```
